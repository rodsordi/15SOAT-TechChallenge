...
---
--- Table: inventoryMaterial (Base table for JOINED inheritance)
---
CREATE TABLE garage.inventoryMaterial (
                                  id UUID PRIMARY KEY,
                                  name VARCHAR(55) NOT NULL,
...
COMMENT ON COLUMN garage.inventoryMaterial.id IS 'Material id. Owner: db';
COMMENT ON COLUMN garage.inventoryMaterial.name IS 'Inventory name. Owner: self';
COMMENT ON COLUMN garage.inventoryMaterial.price IS 'Inventory price. Owner: self';
COMMENT ON COLUMN garage.inventoryMaterial.quantity_in_stock IS 'Inventory quantity in stock. Owner: self';
COMMENT ON COLUMN garage.inventoryMaterial.created_at IS 'Register created at. Owner: db';
COMMENT ON COLUMN garage.inventoryMaterial.updated_at IS 'Register updated at. Owner: db';

---
--- Table: shop_supply (JOINED subclass of inventoryMaterial)
---
CREATE TABLE garage.shop_supply (
                                    id UUID PRIMARY KEY REFERENCES garage.inventoryMaterial(id)
);

---
--- Table: spare_part (JOINED subclass of inventoryMaterial)
---
...
COMMENT ON COLUMN garage.estimate.id IS 'Estimate id. Owner: db';
COMMENT ON COLUMN garage.estimate.amount IS 'Estimate amount. Owner: self';
COMMENT ON COLUMN garage.estimate.created_at IS 'Register created at. Owner: db';
COMMENT ON COLUMN garage.estimate.updated_at IS 'Register updated at. Owner: db';
...
COMMENT ON COLUMN garage.work_order.estimate_id IS 'Estimate id. Owner: db';
